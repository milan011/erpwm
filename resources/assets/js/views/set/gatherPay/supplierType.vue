<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :key="tableKey" :data="list" border fit highlight-current-row style="width: 100%;">
      <el-table-column :label="$t('supplierType.typeid')" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.typeid }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('supplierType.typename')" align="center">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px;margin:0px auto;">
        <el-form-item :label="$t('supplierType.typename')" prop="typename">
...
<script>
  import { getSupplierTypeList, createSupplierType, updateSupplierType, deleteSupplierType,} from '@/api/supplierType'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getSupplierTypeList(this.listQuery).then(response => {
    ...
    handleModifyStatus(row, status) {
      this.$confirm('确定要删除?', '提示', {
        ...
        deleteSupplierType(this.temp).then((response) => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createSupplierType(this.temp).then((response) => {
            console.log(response.data);
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {       
          ...
          updateSupplierType(tempData).then((response) => {
            console.log(response.data)
            const response_data = response.data
            if(response_data.status){
