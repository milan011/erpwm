<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :key="tableKey" :data="list" border fit highlight-current-row style="width: 100%;">
      <el-table-column :label="$t('salesGLPosting.id')" width="60%" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.id }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('salesGLPosting.area')" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.belongs_to_area.areadescription }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('salesGLPosting.stkcat')" show-overflow-tooltip align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.belongs_to_stock_category.categorydescription }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('salesGLPosting.salestype')" show-overflow-tooltip align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.belongs_to_sale_type.sales_type }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('salesGLPosting.salesglcode')" show-overflow-tooltip align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.belongs_to_chart_master_with_salesgl_code.accountname }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('salesGLPosting.discountglcode')" width="120%" show-overflow-tooltip align="center">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="150px" style="width: 400px;margin:0px auto;">
        ...
        <el-form-item :label="$t('salesGLPosting.stkcat')" prop="stkcat">
          <el-select 
            v-model="temp.stkcat" 
            class="filter-item" 
            filterable 
        ...
        <el-form-item :label="$t('salesGLPosting.salestype')" prop="salestype">
          <el-select 
        ...
        <el-form-item :label="$t('salesGLPosting.salesglcode')" prop="salesglcode">
          <el-select 
        ...
        <el-form-item :label="$t('salesGLPosting.discountglcode')" prop="discountglcode">
          <el-select 
...
<script>
  import { getSalesGLPostingList,  createSalesGLPosting, updateSalesGLPosting, deleteSalesGLPosting} from '@/api/salesGLPosting'
...
export default {
  ...
  methods: {
    getList() {
      ...
      getSalesGLPostingList(this.listQuery).then(response => {
    ...
    handleSetRate(row){
      getTaxRates(row).then(response => {
        console.log(response.data)
        // return false
        this.salesGLPostingList = response.data
        this.salesGLPostingName = row.area
        setTimeout(() => {
    ...
    setTaxRateDel(){
      console.log(this.salesGLPostingList)
      // return false
      setTaxRates(this.salesGLPostingList).then(response => {
    ...
    handleModifyStatus(row, status) {
      this.$confirm('确定要删除?', '提示', {
        ...
        deleteSalesGLPosting(this.temp).then((response) => {
          // console.log(response.data);
          if(!response.data.status){
            this.$notify({
              title: '失败',
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {       
          ...
          updateSalesGLPosting(tempData).then((response) => {
            console.log(response.data)
            // return false
            const response_data = response.data
            if(response_data.status){
