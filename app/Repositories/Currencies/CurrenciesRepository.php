...
{
    //默认查询数据
    protected $select_columns = ['id', 'currency', 'currabrev', 'country', 'hundredsname', 'decimalplaces', 'rate', 'webcart', ''];

    // 根据ID获得信息
    public function find($id)
    {
        return Currencies::select($this->select_columns)
            ->findOrFail($id);
    }
    ...
    {
        $query = new Currencies(); // 返回的是一个Order实例,两种方法均可
    ...
    public function create($requestData)
    {
        ...
        try {
            ...
            $tax_categories = new Currencies(); //税目
    ...
    {
        // dd($requestData->all());
        $info = Currencies::select($this->select_columns)->findorFail($id); //获取信息
    ...
    {
        return Currencies::where('taxcatname', $taxcatname)->where('status', '1')->first();
    }
}
