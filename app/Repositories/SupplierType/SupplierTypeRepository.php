...
class SupplierTypeRepository implements SupplierTypeRepositoryInterface
{
    ...
    {
        return SupplierType::select($this->select_columns)->with('hasManyTaxGroupTaxes')
            ->findOrFail($id);
    }
    ...
    {
        $query = new SupplierType(); // 返回的是一个Order实例,两种方法均可
    ...
    public function update($requestData, $id)
    {
        ...
        $info = SupplierType::select($this->select_columns)->findorFail($id); //获取信息
    ...
    public function destroy($id)
    {
        DB::beginTransaction();
        try {
            $info         = SupplierType::findorFail($id);
            $info->status = '0';
            $info->save(); //删除税收组
    ...
    {
        return SupplierType::where('typename', $typename)->first();
    }
}
