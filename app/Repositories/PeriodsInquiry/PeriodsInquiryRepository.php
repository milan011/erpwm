<?php
namespace App\Repositories\PeriodsInquiry;
...
class PeriodsInquiryRepository implements PeriodsInquiryRepositoryInterface
{
    //默认查询数据
    protected $select_columns = ['periodno', 'lastdate_in_period'];

    // 根据ID获得信息
    public function find($id)
    {
        return PeriodsInquiry::select($this->select_columns)
            ->findOrFail($id);
    }
    ...
    {
        $query = new PeriodsInquiry(); // 返回的是一个Order实例,两种方法均可
    ...
    public function create($requestData)
    {
        ...
        try {
            ...
            $tax_categories = new PeriodsInquiry(); //税目
    ...
    {
        // dd($requestData->all());
        $info = PeriodsInquiry::select($this->select_columns)->findorFail($id); //获取信息
    ...
    public function destroy($id)
    {
        ...
        try {
            $tax_authrates = new Taxauthrates(); //税率
            $info          = PeriodsInquiry::findorFail($id);
            $info->status  = '0'; //删除税目
            $info->save();
    ...
    {
        return PeriodsInquiry::where('taxcatname', $taxcatname)->where('status', '1')->first();
    }
}
