...
class BankTransController extends Controller {
	 ...
	 * @return \Illuminate\Http\Response
	 */
	public function getUserBankList(Request $request, $id) {

		$userBanks = $this->bankTrans->getUserBankList($id);
		// dd(lastSql());
		// dd($userBanks);
		$bankList = [];
		foreach ($userBanks as $key => $value) {
			$bankList[] = $value->account_id;
		}
		return $this->baseSucceed($respond_data = $bankList, $message = '成功');
		// return $userBanks;
	}
