...
class CompanyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $companys = $this->company->getList($query_list);
    ...
    public function store(Request $request)
    {
        ...
        if ($this->company->isRepeat($request->name)) {
            return $this->baseFailed($message = '数据已存在');
        }

        $info = $this->company->create($request);
        $info->hasOnePackage;
    ...
    {
        // dd($request->all());
        $update_info = $this->company->isRepeat($request->name);

        if ($update_info && ($update_info->id != $id)) {
            return $this->baseFailed($message = '数据已存在');
        }

        $info = $this->company->update($request, $id);
        $info->hasOnePackage;
